...
| 角色 | 说明 |
|---|---|
| super_admin | 超级管理员，所有 KB 均为 admin 权限，可管理用户/部门/系统配置 |
| kb_admin | 知识库管理员，通过部门/个人授权获得 KB 操作权限 |
| user | 普通用户，通过部门/个人授权获得 KB 操作权限 |

### KB 权限（知识库级）

| 级别 | 权限说明 |
|---|---|
| admin | 删除 KB、修改设置、管理部门/用户授权 |
| editor | 上传/删除文档、重建索引 |
| viewer | 查看文档、智能问答 |
...
```
个人授权（kb_user_access）→ 优先级最高
  ↓ 无
部门授权（kb_department_access）→ 默认权限
  ↓ 无
无权限
```

super_admin 对所有 KB 自动为 admin，不受授权表限制。
...
- [x] FastAPI 后端框架
- [x] MySQL 数据库（10 张表）
- [x] 用户与部门管理（CRUD + 角色筛选 + 部门/状态筛选）
- [x] 知识库管理（CRUD + 权限隔离 + 统计显示）
- [x] 文档上传三步向导（三种分块策略可选）
- [x] 文档管理（列表 + 大小/分块数 + 查看/删除）
- [x] 文本分块（固定长度 / 语义分块 / 结构分析）
- [x] Embedding 向量化（多提供商 + 失败重试 + 配置缓存）
- [x] 语义检索 + LLM 问答（超时保护 + 上下文限制）
- [x] Prompt 管理（界面可编辑）
- [x] 智能问答（预设问题 + Markdown 渲染 + 引用标注）
- [x] 模型配置持久化
- [x] 索引重建
- [x] 前端 SPA（12+ 页面）
- [x] 统计存库（文档上传时写入 Document 表，不再实时计算）

### ✅ 第二期 P1 — 质量提升

- [x] JWT 认证 + API 权限中间件（登录/Token/401 拦截）
- [x] 角色权限控制（super_admin / kb_admin / user，前端菜单+按钮可见性）
- [x] 知识库权限体系（部门授权 + 用户个人授权，三级权限：admin/editor/viewer）
- [x] 审计日志持久化（登录/上传/删除/查询等操作记录入库可查）
- [ ] 混合检索（向量 + BM25 + RRF 融合）
